...
func (h *CustomerHandler) DeleteCustomer(c *gin.Context) {
	...
	// Check if customer has associated accounts or loans
	var accountCount int64
	database.DB.Model(&models.Account{}).Where("customer_id = ? and balance > ?", customer.ID, 0).Count(&accountCount)
	if accountCount > 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "cannot delete customer with active accounts"})
		return
	}

	var loanCount int64
	database.DB.Model(&models.Loan{}).Where("customer_id = ?", customer.ID).Count(&loanCount)
	if loanCount > 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "cannot delete customer with active loans"})
		return
	}
